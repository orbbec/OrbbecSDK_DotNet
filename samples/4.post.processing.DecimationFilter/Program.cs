...
namespace Samples.Decimation
{
    class Program
    {
        ...
        {
            Console.Clear();
            Console.WriteLine("Decimation - Starting...");

            using var renderer = new OrbbecRenderer(title: "Decimation");
            ...
            try
            {
                ...
                if (filter == null)
                {
                    Console.WriteLine("The current device does not support DecimationFilter!");
                    return;
                }
                filter.Enable(true);
        ...
        private static void StartStream(Pipeline pipeline, DecimationFilter filter, OrbbecRenderer renderer, int depthTextureIndex, int processedTextureIndex)
        {
            try
            {
                while (_isRunning)
