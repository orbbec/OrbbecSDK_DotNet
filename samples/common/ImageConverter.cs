...
namespace Samples.Common
{
    public class ImageConverter
    {
        ...
        {
            if (y8Data == null)
                throw new ArgumentException(nameof(y8Data));

            int pixelCount = width * height;
            int rgbSize = pixelCount * 3;

            if (y8Data.Length < pixelCount)
                throw new ArgumentException("Invalid Y8 buffer length");
        ...
        {
            if (y16Data == null)
                throw new ArgumentException(nameof(y16Data));

            int pixelCount = width * height;
            int rgbSize = pixelCount * 3;

            if (y16Data.Length < pixelCount * 2)
                throw new ArgumentException("Invalid Y16 buffer length");
