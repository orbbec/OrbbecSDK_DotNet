...
namespace Samples.DispTransform
{
    class Program
    {
        ...
        {
            Console.Clear();
            Console.WriteLine("DispTransform - Starting...");

            using var renderer = new OrbbecRenderer(title: "DispTransform");
            ...
            try
            {
                ...
                DisparityTransform? filter = null;
                foreach (var f in filterList)
                {
                    if (f.Name().Equals("DisparityTransform"))
                    {
                        filter = f.As<DisparityTransform>();
                    }
                }
                if (filter == null)
                {
                    Console.WriteLine("The current device does not support DisparityTransform!");
                    return;
                }
                filter.Enable(true);
        ...
        private static void StartStream(Pipeline pipeline, DisparityTransform filter, OrbbecRenderer renderer, int depthTextureIndex, int processedTextureIndex)
        {
            try
            {
                while (_isRunning)
