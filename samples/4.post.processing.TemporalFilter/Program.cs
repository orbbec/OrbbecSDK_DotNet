...
namespace Samples.Temporal
{
    class Program
    {
        ...
        {
            Console.Clear();
            Console.WriteLine("Temporal - Starting...");

            using var renderer = new OrbbecRenderer(title: "Temporal");
            ...
            try
            {
                ...
                if (filter == null)
                {
                    Console.WriteLine("The current device does not support TemporalFilter!");
                    return;
                }
                filter.Enable(true);
        ...
        private static void StartStream(Pipeline pipeline, TemporalFilter filter, OrbbecRenderer renderer, int depthTextureIndex, int processedTextureIndex)
        {
            try
            {
                while (_isRunning)
